...
关于 auto-memory（Agent 原生记忆）的权威规则见 `references/native-agent-memory.md`。
...
先读 `AGENT_MEMORY.md`，再按需要选择恢复深度。如果安装了 claude-mem，遵循 `references/claude_mem_integration.md` 的分工流程。
...
| 职责 | zsh | claude-mem |
|------|-----|------------|
| 结构化记忆（导航/三层/任务/日志/决策） | ✅ `AGENT_MEMORY.md`、`skill-docs/*` | ❌ 不重复 |
...
详细分工、恢复流程与示例见 `references/claude_mem_integration.md`。`check_structure.py` 会在健康检查中报告 `claude_mem` 检测状态。
...
候选格式和原生记忆（auto-memory）边界见 `references/native-agent-memory.md`。zsh 的记忆是结构化摘要；不要把 claude-mem 的原始会话内容复制进来。
...
- Agent 原生记忆（auto-memory）与增量回写：`references/native-agent-memory.md`
- 日志与归档：`references/session-log-format.md`
- 记忆压缩：`references/memory-compression.md`
- claude-mem 集成：`references/claude_mem_integration.md`
