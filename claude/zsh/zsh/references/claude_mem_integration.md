...
| 项目架构记忆（三层） | ✅ `zsh/PROJECT_MEMORY.md`（热/温/冷） | ❌ 不重复 |
| 当前任务状态 | ✅ `zsh/CURRENT_TASK.md` | ❌ 不重复 |
| 会话进度日志 | ✅ `zsh/SESSION_LOG.md`（小时+天+周） | ❌ 不重复 |
| 架构决策记录 | ✅ `zsh/DECISIONS.md`（ADR） | ❌ 不重复 |
| 历史归档索引 | ✅ `zsh/memory-archive/INDEX.md` | ❌ 不重复 |
...
2. **读 `zsh/PROJECT_MEMORY.md` 热记忆** → 获取项目全局状态
3. **读 `zsh/CURRENT_TASK.md`** → 了解当前任务进度与精确续接位置
4. **检查 `zsh/SESSION_LOG.md`** 最新条目 → 了解最近的工作
...
1. **更新 `zsh/CURRENT_TASK.md`** → 同步任务进度、精确续接位置
2. **追加 `zsh/SESSION_LOG.md`** → 记录本次会话摘要
3. **如有新架构决策，追加 `zsh/DECISIONS.md`**
...
2. 再读 `zsh/CURRENT_TASK.md` → 确认当前相关工作上下文
3. 再读 `zsh/SESSION_LOG.md` → 找到上次修改的会话记录
4. 最后查 claude-mem → 获取具体的代码实现细节

### 场景：用户问"上次我们讨论的架构决策是什么？"

1. 先读 `zsh/DECISIONS.md` → 找到相关 ADR
2. 再读 `zsh/PROJECT_MEMORY.md` 温记忆 → 找到相关模块说明
