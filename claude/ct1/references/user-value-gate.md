...
- **检查要点**：目标用户明确、用户问题明确、用户目标区别于功能列表、信息来源与置信度披露、关键假设分类、高影响低置信度假设升级、用户成功标准可验证、最小用户旅程覆盖入口/核心价值和必要前置条件、体验风险进入任务图、团队覆盖 `user-value` 能力
...
- **触发事件**：`demo_ready`
- **检查要点**：用户知道从哪里开始、核心操作可发现、文案面向目标用户、空状态提供下一步、加载状态清晰、错误反馈可理解、失败后可恢复、权限不足解释原因和可选动作、危险操作有确认/撤销/明确后果、核心目标无多余步骤、默认值/输入格式/校验提示合理
- **发现问题后**：阻断性体验问题退回任务图；非阻断问题记录风险/影响/建议；范围变化由 leader 判断是否用户确认

### Gate C：交付前 User Outcome Gate

- **触发事件**：`acceptance_ready`
- **检查要点**：fresh user 能独立完成核心旅程、所有必需 US 有真实证据、用户最终结果符合预期、关键异常场景可理解且可恢复、已知体验限制披露、Gate A/B 阻断问题关闭、用户价值结论独立于测试和代码审查
- **结论字段**：
